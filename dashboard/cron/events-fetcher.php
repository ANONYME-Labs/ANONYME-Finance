...
echo $query = "INSERT INTO `event_blocks_synced` (blockNumber) VALUES ('".$blockNumber1."');";
...
$blockNumber = hexdec($blockNumber);
    ...
    if($blockNumber>$lastBlock and $value->topics[0] == $levelBuyEv){
    ...
    elseif($blockNumber>$lastBlock and $value->topics[0] == $lostForLevelEv){
    ...
    elseif($blockNumber>$lastBlock and $value->topics[0] == $paidForLevelEv){
...
$query = "UPDATE adminsetting SET ethPiceInUsd=".$ethPrice.", ethPiceInUSDT=".$ethPrice;
mysqli_query( $conn, $query );


$handle = curl_init();
$url = "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin%2Cethereum&vs_currencies=btc";
// Set the url
curl_setopt($handle, CURLOPT_URL, $url);
// Set the result output to be a string.
curl_setopt($handle, CURLOPT_RETURNTRANSFER, true); 
...
//var_dump($output);
curl_close($handle);
$ethPricebtc = $output['ethereum']['btc'];
//updating the admin setting table
$query = "UPDATE adminsetting SET ethPiceInBtc=".$ethPricebtc;
mysqli_query( $conn, $query );
